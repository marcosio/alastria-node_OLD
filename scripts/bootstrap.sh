...
sudo add-apt-repository -y ppa:ethereum/ethereum &&  apt-get update && apt-get install -y solc
...
sudo cp constellation-0.1.0-ubuntu1604/constellation-node /usr/local/bin && chmod 0755 /usr/local/bin/constellation-node
